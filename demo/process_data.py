...
def main():
    ...
    for filename in os.listdir(log_dir):
        ...
        with open(join(log_dir, filename), "rb") as file:
            blockchain_info = pickle.load(file)
            tempo_rodada = blockchain_info[0]
            total_blocos = blockchain_info[1]
            total_miniblocos = blockchain_info[2]
        ...
        print(f"**************  DATA *******************************")
        print(f"tempo da rodada       = {tempo_rodada}")
        print(f"total de blocos       = {total_blocos}")
        print(f"total de miniblocos   = {total_miniblocos}")
        print(f"total de transacoes   = 300")

        tps = (total_miniblocos * 300) / tempo_rodada
        print(f"TPS                   = {tps:.2f}")
        mini_rodada = (total_miniblocos ) / total_blocos
        print(f"miniblocos por rodada   = {mini_rodada:.2f}")
        print(f"***********************************************************")
